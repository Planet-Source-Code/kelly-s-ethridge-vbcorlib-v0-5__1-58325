    ...
    Select Case VarType(y)
        ...
        Case vbObject, vbDataObject
            If TypeOf y Is IComparable Then
                ...
                Exit Function
            Else
                Throw Cor.NewInvalidCastException("Object must implement IComparable interface.")
            End If
        Case Else
...
Public Sub SwapSortItems(ByRef items As SortItems, ByVal i As Long, ByVal j As Long)
    With items.SA
        Select Case .cbElements
            ...
            Case Else
                ' primarily for UDTs
                CopyMemory ByVal mSortItems.Buffer, ByVal .pvData + i * .cbElements, .cbElements
                CopyMemory ByVal .pvData + i * .cbElements, ByVal .pvData + j * .cbElements, .cbElements
                CopyMemory ByVal .pvData + j * .cbElements, ByVal .pvData + i * .cbElements, .cbElements
        End Select
    End With
End Sub
...
Public Sub QuickSortObject(ByRef Keys() As Object, ByVal left As Long, ByVal right As Long)
    Dim i As Long, j As Long, x As Object, Key As IComparable
    Do While left < right
